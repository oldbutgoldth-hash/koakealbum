...
export function GalleryClient({ album, photos }: { album: GalleryAlbum; photos: GalleryPhoto[] }) {
  ...
  useEffect(() => {
    ...
    const update = () => setMobileViewer(media.matches || navigator.maxTouchPoints > 0);
    update();
  ...
  return (
    <main>
      ...
      {currentPhoto && <div className="lightbox" role="dialog" aria-modal="true" aria-label={`ดูภาพ ${lightbox! + 1} จาก ${photos.length}`} onTouchStart={(event) => { touchStart.current = event.touches[0].clientX; }} onTouchEnd={(event) => { if (touchStart.current === null) return; const distance = event.changedTouches[0].clientX - touchStart.current; if (Math.abs(distance) > 45) move(distance > 0 ? -1 : 1); touchStart.current = null; }}>
        ...
        <button className="nav prev" onClick={() => move(-1)} aria-label="รูปก่อนหน้า">←</button><div className={`lightbox-stage ${mobileViewer ? "mobile-preview-stage" : ""}`} style={mobileViewer ? { backgroundImage: `url(${currentPhoto.src})` } : undefined} role={mobileViewer ? "img" : undefined} aria-label={mobileViewer ? currentPhoto.alt : undefined}>{!mobileViewer && <img key={currentPhoto.id} src={lightboxPreview || currentPhoto.src} alt={currentPhoto.alt} />}</div><button className="nav next" onClick={() => move(1)} aria-label="รูปถัดไป">→</button><div className="lightbox-bottom"><p>{currentPhoto.filename}</p><span>ปัดซ้าย–ขวา หรือใช้ปุ่มลูกศรเพื่อดูรูปถัดไป</span></div>
      </div>}
